...
class PlayerLogTab(QWidget):
    ...
    def _init_file_info_group(self) -> None:
        ...
        # Removed max height to allow flexible height
        # file_info_group.setMaximumHeight(120)
    ...
    def _init_statistics_group(self) -> None:
        ...
        self.total_lines_label.setStyleSheet("color: #00FF00;")
        self.info_label.setStyleSheet("color: #00FF00;")
        self.warnings_label.setStyleSheet("color: #FF8C00;")
        self.mod_issues_label.setStyleSheet("color: #FF8C00;")
        self.errors_label.setStyleSheet("color: #FF0000;")
        self.exceptions_label.setStyleSheet("color: #FF0000;")
        self.keybind_label.setStyleSheet("color: #EEFF00;")
    ...
    def _init_quick_navigation_group(self) -> None:
        ...
        self.goto_error_prev_btn = create_nav_button(
            "Previous Error",
            "Jump to previous error entry",
            lambda: self.goto_previous_pattern(
                r"(?i)(error|failed|fatal|critical|\[E\])"
            ),
        )
        self.goto_error_next_btn = create_nav_button(
            "Next Error",
            "Jump to next error entry",
            lambda: self.goto_first_pattern(r"(?i)(error|failed|fatal|critical|\[E\])"),
        )
        self.goto_warning_prev_btn = create_nav_button(
            "Previous Warning",
            "Jump to previous warning entry",
            lambda: self.goto_previous_pattern(r"(?i)(warning|warn|deprecat|\[W\])"),
        )
        self.goto_warning_next_btn = create_nav_button(
            "Next Warning",
            "Jump to next warning entry",
            lambda: self.goto_first_pattern(r"(?i)(warning|warn|deprecat|\[W\])"),
        )
        self.goto_exception_prev_btn = create_nav_button(
            "Previous Exception",
            "Jump to previous exception entry",
            lambda: self.goto_previous_pattern(r"Exception|Error:"),
        )
        self.goto_exception_next_btn = create_nav_button(
            "Next Exception",
            "Jump to next exception entry",
            lambda: self.goto_first_pattern(r"Exception|Error:"),
        )
        self.goto_mod_prev_btn = create_nav_button(
            "Previous Mod Issue",
            "Jump to previous mod issue entry",
            lambda: self.goto_previous_pattern(r"\[.*\]"),
        )
        self.goto_mod_next_btn = create_nav_button(
            "Next Mod Issue",
            "Jump to next mod issue entry",
            lambda: self.goto_first_pattern(r"\[.*\]"),
        )
        self.goto_keybind_prev_btn = create_nav_button(
            "Previous Keybind Conflict",
            "Jump to previous keybind conflict entry",
            lambda: self.goto_previous_pattern(r"key binding conflict"),
        )
        self.goto_keybind_next_btn = create_nav_button(
            "Next Keybind Conflict",
            "Jump to next keybind conflict entry",
            lambda: self.goto_first_pattern(r"key binding conflict"),
        )
        self.goto_info_prev_btn = create_nav_button(
            "Previous Info",
            "Jump to previous info entry",
            lambda: self.goto_previous_pattern(
                r"(?i)(info|initialized|loaded|start(ed)?|done|success)"
            ),
        )
        self.goto_info_next_btn = create_nav_button(
            "Next Info",
            "Jump to next info entry",
            lambda: self.goto_first_pattern(
                r"(?i)(info|initialized|loaded|start(ed)?|done|success)"
            ),
        )
        self.scroll_to_end_btn = create_nav_button(
            "Scroll to End", "Scroll to the end of the log display", self.scroll_to_end
        )

        buttons = [
            self.goto_error_prev_btn,
            self.goto_error_next_btn,
            self.goto_warning_prev_btn,
            self.goto_warning_next_btn,
            self.goto_exception_prev_btn,
            self.goto_exception_next_btn,
            self.goto_mod_prev_btn,
            self.goto_mod_next_btn,
            self.goto_keybind_prev_btn,
            self.goto_keybind_next_btn,
            self.goto_info_prev_btn,
            self.goto_info_next_btn,
            self.scroll_to_end_btn,
        ]
    ...
    def apply_filter(self) -> None:
        ...
        for line in lines:
            ...
            if filter_text == "All Entries":
                ...
                filtered_lines.append(line)
            elif filter_text == "Warnings & Errors" and (
    ...
    def load_log_from_link(self) -> None:
        ...
        if ok and url:
            # Placeholder for actual URL loading logic
            show_information(f"Loading log from URL: {url}")
            # Implement actual loading from URL if needed
