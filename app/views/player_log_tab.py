...
class LogHighlighter(QSyntaxHighlighter):
    def __init__(self, parent: QObject) -> None:
        ...
        self.info_format = self._make_format("#00FF0D")
        self.mod_format = self._make_format("#1100FFFF")
        ...
        self.search_format.setUnderlineColor(QColor("#E5FF00"))
    ...
    def set_search_highlight_color(self, color: QColor) -> None:
        """Set the color used for search term highlighting."""
...
class PlayerLogTab(QWidget):
    ...
    def __init__(self, settings_controller: SettingsController) -> None:
        ...
        self.init_ui()
        self.auto_load_default_log_checkbox.setChecked(
            self.settings_controller.settings.auto_load_default_log
        )
        self.auto_load_default_log_checkbox.toggled.connect(
            self._on_auto_load_default_log_toggled
        )
        self.player_log_path = self._get_player_log_path()
        # Set checkbox state from settings
        if self.settings_controller.settings.auto_load_default_log:
            self.load_log()

    def _on_auto_load_default_log_toggled(self, checked: bool) -> None:
        self.settings_controller.settings.auto_load_default_log = checked
    ...
    def pick_search_highlight_color(self) -> None:
        """Open a color picker dialog to select the search highlight color."""
    ...
    def init_ui(self) -> None:
        ...
        self.tabs.addTab(file_info_tab, "File Info")

        # Controls group
        controls_group = QGroupBox("Controls")
        controls_layout = QVBoxLayout(controls_group)

        # New checkbox for auto load default log
        self.auto_load_default_log_checkbox = QCheckBox(
            "Auto Load Default Log on Startup"
        )
        self.auto_load_default_log_checkbox.setToolTip(
            "If checked, the default log will be loaded automatically on startup."
        )
        controls_layout.addWidget(self.auto_load_default_log_checkbox)

        # New buttons for loading logs
        load_buttons_layout = QHBoxLayout()
        self.load_default_button = QPushButton("Load Default Log")
        self.load_default_button.setToolTip("Load the default current game log")
        ...
        self.auto_refresh_interval_input = QSpinBox()
        self.auto_refresh_interval_input.setRange(1, 3600)  # 1 second to 1 hour
        self.auto_refresh_interval_input.setValue(20)
        self.auto_refresh_interval_input.setSuffix(" s")
        self.auto_refresh_interval_input.setToolTip(
            "Set auto-refresh interval in seconds"
        )
        self.auto_refresh_interval_input.valueChanged.connect(
            self._update_autorefresh_interval
        )
        controls_layout.addWidget(self.auto_refresh_interval_input)

        self.pause_refresh_button = QPushButton("Pause Auto-Refresh")
        self.pause_refresh_button.setCheckable(True)
        self.pause_refresh_button.setToolTip("Toggle auto-refresh on/off")
        ...
        nav_layout.addWidget(self.goto_error_prev_btn, 0, 1)
        nav_layout.addWidget(self.goto_warning_btn, 1, 0)
        nav_layout.addWidget(self.goto_warning_prev_btn, 1, 1)
        nav_layout.addWidget(self.goto_exception_btn, 2, 0)
        nav_layout.addWidget(self.goto_exception_prev_btn, 2, 1)
        nav_layout.addWidget(self.goto_mod_btn, 3, 0)
        nav_layout.addWidget(self.goto_mod_prev_btn, 3, 1)
        nav_layout.addWidget(self.goto_keybind_btn, 4, 0)
        nav_layout.addWidget(self.goto_keybind_prev_btn, 4, 1)
        nav_layout.addWidget(self.goto_info_btn, 5, 0)
        nav_layout.addWidget(self.goto_info_prev_btn, 5, 1)
    ...
    def _update_autorefresh_interval(self, value: int) -> None:
    ...
    def _analyze_log_content(self, content: str) -> None:
        ...
        for line in lines:
            ...
            if "key binding conflict" in line_lower:
                self.log_stats["keybind_conflicts"] += 1

            if re.search(
                r"(?i)\b(info|initialized|loaded|start(ed)?|done|success)\b", line
            ):
    ...
    def apply_filter(self) -> None:
        ...
        # Precompile mod_filter for efficiency if provided
        mod_filter_re = (
        ...
        for line in lines:
            ...
            if filter_type == "All Entries":
                ...
                include_line = any(
                    ...
                    for keyword in ["[e]", "error", "failed", "fatal"]
                )
                ...
                include_line = (
                    "[" in line
                    and "]" in line
                    and any(k in line_lower for k in ["error", "warning", "exception"])
                ) or (
                    "mod" in line_lower
                    and any(k in line_lower for k in ["conflict", "issue"])
                )
    ...
    def toggle_auto_refresh(self, enabled: bool) -> None:
        ...
        if enabled:
            if self.timer:
                self.timer.start()
            self.pause_refresh_button.setText("Pause Auto-Refresh")
        else:
            if self.timer:
                self.timer.stop()
            self.pause_refresh_button.setText("Resume Auto-Refresh")
