...
class LogHighlighter(QSyntaxHighlighter):
    def __init__(self, parent: QObject) -> None:
        ...
        self.patterns = [
            ...
            (re.compile(r"\[([^\]]+)\]"), self.mod_issue_format),
...
class PlayerLogTab(QWidget):
    ...
    def _init_statistics_group(self) -> None:
        ...
        self.info_label = create_stat_button("Infos: 0", "#00FF00", "Infos Only")
        self.keybind_label = create_stat_button(
            "Keybinds: 0", "#EEFF00", "Keybinds Only"
        )
        ...
        self.all_issues_label = create_stat_button(
            "All Issues: 0", "#FF0000", "All Issues"
        )
    ...
    def _update_file_info(self) -> None:
        ...
        if self.player_log_path is None:
            ...
            if self.current_log_content:
                url = self.url
                if url:
                    self.file_path_label.setText(f"Path: Loaded from URL: {url}")
                else:
                    self.file_path_label.setText("Path: Loaded from URL")
                self.file_size_label.setText(
                    f"Size: {len(self.current_log_content):,} bytes"
                )
    ...
    def apply_filter(self) -> None:
        ...
        for line in lines:
            ...
            if filter_text == "All Entries":
            ...
            elif filter_text == "Infos Only" and self._info_pattern.search(line):
                filtered_lines.append(line)
            elif filter_text == "Keybinds Only" and keybind_pattern.search(line):
                filtered_lines.append(line)
                ...
                filtered_lines.append(line)
            elif filter_text == "All Issues" and (
    ...
    def load_log_from_link(self) -> None:
        ...
        if ok and url:
            try:
                ...
                self.scroll_to_end()
                show_information(
                    title="Log loaded successfully from URL",
                    text=f"{url}",
                )
            except Exception as e:
                show_warning(
                    title="Failed to load log from URL",
                    text=f"Failed due to error: {e}",
                )
    ...
    def _analyze_log_content(self, content: str) -> None:
        ...
        # Use individual patterns pattern for better matching
        info_pattern = re.compile(
            r"\b(info|initialized|loaded|start(ed)?|done|success)\b", re.IGNORECASE
        )
        keybind_pattern = re.compile(r"key binding conflict", re.IGNORECASE)
        mod_issue_pattern = re.compile(
            r"(\[.*\].*(error|warning|exception))|(mod.*(conflict|issue))",
            re.IGNORECASE,
        )
        warning_pattern = re.compile(
            r"\b(warning|warn|deprecat)\b|\[W\]", re.IGNORECASE
        )
        error_pattern = re.compile(r"\b(error|failed|fatal)\b|\[E\]", re.IGNORECASE)
        exception_pattern = re.compile(r"exception", re.IGNORECASE)

        for line in lines:
            if info_pattern.search(line):
                self.log_stats["infos"] += 1
            if keybind_pattern.search(line):
                self.log_stats["keybinds"] += 1
            if mod_issue_pattern.search(line):
    ...
    def _update_statistics(self) -> None:
        ...
        self.info_label.setText(f"Infos: {self.log_stats['infos']}")
        self.keybind_label.setText(f"Keybinds: {self.log_stats['keybinds']}")
        ...
        self.all_issues_label.setText(
            f"All Issues: {self.log_stats['errors'] + self.log_stats['warnings'] + self.log_stats['exceptions'] + self.log_stats['mod_issues'] + self.log_stats['keybinds']}"
        )
