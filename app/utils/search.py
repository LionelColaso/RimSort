...
class SearchThread(QThread):
    ...
    def _process_file(self, file: str, root: str) -> int:
        """
        ...
        Returns:
            int: 1 if file matches criteria, 0 otherwise
        """
        ...
        # If search type is ".xml Extensions Only", skip files not ending with .xml early
        if self.search_options.get(
            "search_type"
        ) == ".xml Extensions Only" and not file.endswith(".xml"):
            return 0
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for file, root in files_to_process:
                    if self.isInterruptionRequested():
                        break
                    futures.append(executor.submit(self._process_file, file, root))
...
class SearchTool(QMainWindow):
    ...
    def start_search(self) -> None:
        ...
        if not folder:
            show_warning(title="Warning", text="No valid search path provided.")
    ...
    def check_search_criteria(self, file: str, file_path: str) -> bool:
        ...
        if self.search_options["search_type"] == "File and Folder Names":
            ...
            if self.search_options.get("case_sensitive"):
                return self.search_text.text() in file
            else:
            ...
            try:
                ...
                search_bytes = (
                    self.search_text.text().encode()
                    if self.search_options.get("case_sensitive")
                    else self.search_text.text().lower().encode()
                )
                ...
                with open(file_path, "rb") as f:
                    ...
                    try:
                        # Read in chunks and check for search_bytes
                        chunk_size = 1024 * 1024  # 1MB
                        found = False
                        offset = 0
                        while offset < mmapped_file.size():
                            end = min(offset + chunk_size, mmapped_file.size())
                            ...
                            if self.search_options.get("case_sensitive"):
                                ...
                                if search_bytes in chunk.lower():
                                    found = True
                                    break
                            offset += chunk_size
                        return found
        ...
        if self.mod_select_combo.currentText() == "All Mods":
            ...
            with open(self.search_txt_path, "a", encoding="utf-8") as search_txt_file:
                if local_mods_target and local_mods_target != "":
                    ...
                    search_txt_file.write(abs_local + "\n")
                    logger.debug(f"Added local mods path: {abs_local}")
                if workshop_mods_target and workshop_mods_target != "":
                    abs_workshop = os.path.abspath(workshop_mods_target)
                    search_txt_file.write(abs_workshop + "\n")
                    logger.debug(f"Added workshop mods path: {abs_workshop}")
        else:
            with open(self.search_txt_path, "a", encoding="utf-8") as search_txt_file:
                if hasattr(self.mods_panel.active_mods_list, "uuids"):
                    for uuid in self.mods_panel.active_mods_list.uuids:
                        ...
                        if mod_data and "path" in mod_data:
                            ...
                            if mod_path and os.path.exists(mod_path):
                                ...
                                search_txt_file.write(abs_mod_path + "\n")
                                logger.debug(f"Added active mod path: {abs_mod_path}")
                            else:
                                logger.warning(
