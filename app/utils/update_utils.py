...
class UpdateManager(QObject):
    ...
    def do_check_for_update(self) -> None:
        ...
        2. Checks if elevation is needed for Windows
        3. Fetches latest release information from GitHub
        4. Compares versions and prompts user if update is available
        5. Downloads and extracts the update if user confirms
        6. Launches the appropriate update script for the platform
        """
        ...
        # Check if elevation is needed only for windows so .msi installer can be used
        ...
        try:
            if current_version == "Unknown version":
                # If version is unknown, assume up to date to prevent false update prompts
